...
class Aggregator {
    ...
    public static bool removeUnfinishedAggregations = false;
    public static List<IOutputStatementType> Aggregate(List<IStatementType> statementTable)
    {
        ...
        foreach (var group in groupedStatements) {
            ...
            foreach(var type in types) {
                ...
                if(outputStatement != null) {
                    if (removeUnfinishedAggregations && outputStatement.ToDate > DateTime.Now) {
                        Console.WriteLine("Skipping this Aggregation because this aggregation has an end date \"" +outputStatement.ToDate.ToString() + "\" greater than \"" + DateTime.Now.ToString() + "\"" );
                    }
                    else {
